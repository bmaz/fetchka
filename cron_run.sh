...
LAST_FILE="last_tweets_2022-03-03T08.csv"
NOW=$(date +"%Y-%m-%dT%H")
#NEW_FILE="last_tweets_${NOW}.csv"
NEW_FILE="last_tweets_2022-03-03T10.csv"
LOG_FILE="logs.log"
...
#docker run -ti --name minet-users -v /rex/local/bmtweet/factcheck/:/usr/src/app minet twitter user-tweets --rcfile /usr/src/app/.minetrc --ids user_numeric_id /usr/src/app/twitter_unique_handles_2022_02_28.csv -o /usr/src/app/${NEW_FILE} --min-date $(date --date='-31days' +'%Y-%m-%d')
...
docker run -ti --name minet-attrition -v /rex/local/bmtweet/factcheck/:/usr/src/app minet twitter attrition --rcfile /usr/src/app/.minetrc --ids --user user_id id /usr/src/app/tmp_attrition.csv -o /usr/src/app/tmp_tmp_attrition.csv

# Add result to final file
xsv select "tweet_current_status,${COLUMNS}" tmp_tmp_attrition.csv | xsv behead >> past_tweets_users.csv
